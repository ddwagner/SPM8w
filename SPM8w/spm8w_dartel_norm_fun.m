...
% DARTEL template
%if ~isempty(job.template{1})   %===DDW EDIT===
if strcmp(job.normtok,'DARTEL')
    Nt     = nifti(job.template{1});
    do_aff = true;
else
    %Nt     = nifti(tpm);     %===DDW EDIT===
    Nt     = nifti(job.template{1});
    do_aff = false;
end
...
if isfield(job.data,'subj') || isfield(job.data,'subjs'),
    ...
    for i=1:numel(subj),
        ...
        fprintf('** "%s" **\n', nam);
        out{i} = deal_with_subject(subj(i).flowfield,subj(i).images,K, mat,dim,M,job.preserve,job.fwhm,mat_intent,job.normtok);
    end
...
function out = deal_with_subject(Pu,PI,K,mat,dim,M,jactransf,fwhm,mat_intent,normtok) %===DDW EDIT===
...
for m=1:numel(PI),
    ...
    if jactransf,
        if fwhm==0,
            %NO.dat.fname=fullfile(pth,['mw' nam ext]);%===DDW EDIT=== 
            NO.dat.fname=fullfile(pth,['m',normtok nam ext]);%===DDW EDIT=== 
        else
            %NO.dat.fname=fullfile(pth,['smw' nam ext]);%===DDW EDIT=== 
            NO.dat.fname=fullfile(pth,['sm', normtok nam ext]);%===DDW EDIT=== 
        end
        ...
        if fwhm==0,
            %NO.dat.fname=fullfile(pth,['w' nam ext]);%===DDW EDIT=== 
            NO.dat.fname=fullfile(pth,[normtok nam ext]);%===DDW EDIT=== 
        else
            %NO.dat.fname=fullfile(pth,['sw' nam ext]);%===DDW EDIT===
            NO.dat.fname=fullfile(pth,['s' normtok nam ext]);%===DDW EDIT=== 
        end
    end
    ...
    %----------------------------------------------------------------------
    %fprintf('%s',nam); drawnow;%===DDW EDIT===
    fprintf('Normalizing file %s.nii with DARTEL:',nam);%===DDW EDIT===
    for j=1:size(NI.dat,4),
        ...
        for k=1:size(NI.dat,5),
            for l=1:size(NI.dat,6),
                ...
                NO.dat(:,:,:,j,k,l) = f;
                %fprintf('\t%d,%d,%d', j,k,l); drawnow;%===DDW EDIT===
                fprintf('.');%===DDW EDIT===
            end
        end
    end

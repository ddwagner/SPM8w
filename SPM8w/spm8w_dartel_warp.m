...
%spm_progress_bar('Init',n2,'Registering'); %===DDW EDIT===
fprintf('Inital Registration...\n');
numits = 0;
...
for i=1:n2,
    ...
    pthtmp = pth(1:strfind(pth,spm_str_manip(pth,'t'))-2);
    fprintf('\nWorking on subject: %s file: %s', spm_str_manip(pthtmp,'t'),nam);
    ...
    for it=1:numel(st.param),
        ...
        for j=1:param.its,
            fprintf('\nIteration %d of 18',iter); %===DDW EDIT===
            it0 = it0 + 1;
            [u,ll] = dartel3(u,f,g,prm);
            %fprintf('%d \t%g\t%g\t%g\t%g\n',...
            %    it0,ll(1),ll(2),ll(1)+ll(2),ll(3));   %===DDW EDIT====
            iter = iter+1; %===DDW EDIT ===
            %spm_progress_bar('Set',i-1 + it0/numits); %===DDW EDIT ===
        end
    end
    fprintf('\n'); %===DDW EDIT===
    create(NU);

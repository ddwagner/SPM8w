function spm8w_designtool(varargin) 
% ==============================================================================
% SPM8w r5236
...
% Created: March, 2011 - DDW
% ==============================================================================
% function spm8w_designtool('type', 'dsb_para_file')
%
% spm8w_designtool performs one of three tasks depending on how it is
% called. 
% 
% Type: 'SEARCH'
% Design tool can SEARCH for search for an optimal design matrix
% iterating through randomizations and null event ratios until the one with
% the lowest collinearity between conditions is found.
% 
% Type: 'CHECK'
% Design tool can also CHECK the collinearity on a pre-defined design
% matrix. Useful for testing collinearity on models which require 
% customization (multiple components to a trial, unbalanced conditions)
% 
% Type: 'BUILD'
% Design tool can also BUILD an fMRI design by taking as input stimulus
% files and outputting a numbered list of image files randomized and
% jittered with null event trials according to the input design matrix.
% 
% To use: 
% spm8w_designtool('type', 'dsb_para_file')
% ==============================================================================
% CHANGE LOG:
% -First version - March 2011
% -First pass at windows compatbility - April 2012
% -Changed behavior so fixed (i.e., slow er) or block designs can (in theory) 
%  be made. - April 2012
% =======1=========2=========3=========4=========5=========6=========7=========8
...
switch (nargin)
  case 0 
    error('Please specify ''type'' and optinally the full path to the parameter file e.g. spm8w_designtool(''search'')');
  case 1 
    d.type = varargin{1};
    %Check for spelling.
    if ~strcmp(d.type,'search') && ~strcmp(d.type,'SEARCH') ...
       && ~strcmp(d.type,'check') && ~strcmp(d.type,'CHECK') ...
       && ~strcmp(d.type,'build') && ~strcmp(d.type,'BUILD') 
        error('Invalid ''type'' check your spelling? Valid types are search, check and build');
    end
    if ~strcmp(d.type,'check') && ~strcmp(d.type,'CHECK')
        design_para_file = spm_select(1,'^D_.*\.m$','Please select your Design parameter file.',[],[pwd,'/SCRIPTS']);
        cd(['/',spm_str_manip(design_para_file,'h')]);
        eval_this        = spm_str_manip(design_para_file,'tr');
        eval(eval_this);
        cd(d.root); 
    else
        d.root = pwd; 
  ...
  case 2
    d.type           = varargin{1};
    design_para_file = varargin{2}
    try
        cd(spm_str_manip(design_para_file,'h'));
        eval(spm_str_manip(design_para_file,'tr'));
        cd(d.root); 
    catch
        error(['Please be sure to provide the full path to your Design parameters file, or leave ' ...
              'this argument blank to select via a window.']);
    end
    otherwise
    error('You''ve specified too many inputs. Only specify ''type'' and optinally the D_para.m file.');
end
...
if ~strcmp(d.type,'check') && ~strcmp(d.type,'CHECK')
    if ~isdir(fullfile(d.root,'DESIGN'));    [s,w]=system(sprintf('mkdir "%s"',fullfile(d.root,'DESIGN')));    end
    if ~isdir(d.design_dir);                 [s,w]=system(sprintf('mkdir "%s"',d.design_dir));        end
    if ~isdir(fullfile(d.design_dir,'MAT')); [s,w]=system(sprintf('mkdir "%s"',fullfile(d.design_dir,'MAT'))); end
    if ~isdir(d.output_dir);                 [s,w]=system(sprintf('mkdir "%s"',d.output_dir));        end
end

%%%CLEANUP
clear varargin eval_this design_para_file s w %keep d
...
switch(d.type)
    case{'search','SEARCH'}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Loop through d.iterate and d.nullratio  
    ...
    if strcmp(d.nullratio,'fixed')
        nulls = 1;
    else
        nulls = length(d.nullratio);
    end
    for i = 1:nulls 
        effstudy = 1; effruns  = 1;
        for ii = 1:d.iterate
           %%%GENERATE MATRIX
           d.design{i}.matrix{ii} = generate_design(d.cond_specs, d.nses, d.nullbeg, d.nullend, d.nullratio, d.nullfix, d.counter);%d.nullratio(i) for normal operation april2012
           %%%REPORT STATS ON MATRIX
           actual_ratio = (sum(d.design{i}.matrix{ii}{1}==666)-d.nullbeg-d.nullend)/(length(d.design{i}.matrix{ii}{1})-d.nullbeg-d.nullend);    
           fprintf('=Block:%d Iteration:%2.0f   Run Length:%d \n=Desired Nullratio:%1.2f Actual Nullratio:%1.3f',i,ii,length(d.design{i}.matrix{ii}{1}),d.nullratio(i),actual_ratio); 
           fprintf('\n');   
           %%%GENERATE SPM DESIGN MATRIX  
           [d.design{i}.SPMstudy{ii}, d.design{i}.SPMruns{ii}] = design_maker(d.design{i}.matrix{ii}, d.cond_specs, d.nses, d.TR, d.hpf, d.autocorr);           
           %%%GENERATE AND REPORT DESIGN EFFICIENCY
           d.design{i}.effstudy{ii} = design_efficiency(d.design{i}.SPMstudy{ii});
           eff_reporter(d.design{i}.effstudy{ii}, d.design{i}.SPMstudy{ii}.Sess.U);
           if d.nses > 1
                for x = 1:length(d.design{i}.SPMruns{ii})
                    d.design{i}.effruns{ii}{x} = design_efficiency(d.design{i}.SPMruns{ii}(x));
                    maxruns(x)                 = max(d.design{i}.effruns{ii}{x});
                end
                eff_reporter(d.design{i}.effruns{ii});
           else
           fprintf('\n');   
           end
           %%%CHECK IF TOP DOG STUDY FOR STUDY AND RUNS
           if max(d.design{i}.effstudy{ii}) < effstudy
                mindex_study(i) = ii;
                effstudy        = max(d.design{i}.effstudy{ii});
           end
    ...
    for i = 1:nulls
        %%% ASSIGN WINNER ACCORDING TO D.CRITERIA
        if strcmp(d.criteria,'study')
            mindex = mindex_study(i);
        elseif strcmp(d.criteria,'runs')
            mindex = mindex_runs(i);
        else
            error('Criteria is neither study nor runs, check your spelling for d.criteria');
        end
        %GET THE RUN WIDE MIN/MAX
        for x = 1:length(d.design{i}.effruns{mindex})
            maxruns(x) = max(d.design{i}.effruns{mindex}{x});
        end
        %CALCULATE THE ACTUAL RATIO FOR THE WINNER
        actual_ratio = (sum(d.design{i}.matrix{mindex}{1}==666)-d.nullbeg-d.nullend)/(length(d.design{i}.matrix{mindex}{1})-d.nullbeg-d.nullend);
        fprintf(['\n==========Best design for nullratio of %1.2f is\n' ...
                '===Iteration #:%d Run Length:%d Actual nullratio:%1.2f\n' ...
                '===Design Wide: MinR:%1.3f | MaxR:%1.3f\n' ...
                '===Run Wide   : MinR:%1.3f | MaxR:%1.3f\n'], d.nullratio(i), mindex, ...
                length(d.design{i}.matrix{mindex}{1}), actual_ratio, ... 
                min(d.design{i}.effstudy{mindex}), max(d.design{i}.effstudy{mindex}),...
                min(maxruns), max(maxruns)); 
        %SAVE BEST DESIGN 
        dfilename = [d.design_pfx,'_',num2str(mindex),'_',num2str(d.nullratio(i)),'.mat'];
        D_RUNS    = d.design{1}.matrix{mindex};
        save (dfilename, 'D_RUNS');
        [s,w]     = system(spm8w_osbabel(sprintf('mv %s %s',dfilename,fullfile(d.design_dir,'MAT'))));
        fprintf('===Design saved to file %s in directory:\n===%s\n',dfilename, [d.design_dir,'/MAT']) 
        %%% Spit Time
    ...
    dmat_file = spm_select(1,'.mat$','Please select your Design matrix or SPM file.',[],d.root);
    dmat = load(dmat_file);
    if(isfield(dmat,'SPM'))
    ...
    dmf = spm_select(1,['^',d.design_pfx,'_.*\.mat$'],'Please select your Design matrix file.',[],d.design_dir);
    dmf = load(dmf)
    dmf = dmf.D_RUNS
    length(dmf);
    length(dmf{1});
    pause
    stimcounter(1:length(d.cond_specs)) = 1;
    ...
    for i = 1:length(dmf)  %for each run
        ...
        for ii = 1:length(dmf{i})
            if dmf{i}(ii) == 666
                [s,w]=system(spm8w_osbabel(sprintf('cp "%s/"%s "%s/RUN%02d/"%04d_fixation.mp4',d.input_dir,d.null_filename,d.output_dir,i,ii)));
            else
%                 [namesize, tmp] = size(d.cond_specs{dmf{i}(ii),4});
%                 if namesize > 1
%                     if stimcounter(dmf{i}(ii)) > namesize
%                         stimcounter(dmf{i}(ii)) = 1;
%                         [s,w]=unix(['cp ',d.input_dir,'/',d.cond_specs{dmf{i}(ii),3}{stimcounter(dmf{i}(ii))},' ',d.output_dir,'/RUN',num2str(i),'/',num2str(ii),'_cond',num2str(dmf{i}(ii)),'.jpg']);
%                     else
% %                         d.cond_specs{dmf{i}(ii),3}
% %                          d.cond_specs{dmf{i}(ii),3}{stimcounter(dmf{i}(ii))}
%                         [s,w]=unix(['cp ',d.input_dir,'/',d.cond_specs{dmf{i}(ii),3}{stimcounter(dmf{i}(ii))},' ',d.output_dir,'/RUN',num2str(i),'/',num2str(ii),'_cond',num2str(dmf{i}(ii)),'.jpg']);
%                     end
%                     stimcounter(dmf{i}(ii)) = stimcounter(dmf{i}(ii))+1;
               % else
                   % [s,w]=unix(['cp ',d.input_dir,'/',d.cond_specs{dmf{i}(ii),4},' ',d.output_dir,'/RUN',num2str(i),'/',num2str(ii),'_cond',num2str(dmf{i}(ii)),'.jpg']);
                   %get files in dir, randomly chose 2 and reset counter
                   dirname = fullfile(d.input_dir,d.cond_specs{dmf{i}(ii),4});
                   curdir  = dir([dirname,filesep,'*.mp4']);
                   curidx  = eval(['count',num2str(dmf{i}(ii))]);
                   if strcmp(d.cond_specs{dmf{i}(ii),4},'NULL')
                       currand = randnull;
                   elseif strcmp(d.cond_specs{dmf{i}(ii),4},'CATCH')
                       currand = randcatch;
                   else
                       currand = randchars;
                   end                      
                   [s,w]=system(spm8w_osbabel(sprintf('cp "%s/"%s "%s/RUN%02d/"%04d_%s_%s',dirname,curdir(currand(curidx)).name,d.output_dir,i,ii,d.cond_specs{dmf{i}(ii),4},curdir(currand(curidx)).name)));  
    ...
    %Step 2-Copy images from d.input to d.output 
...
% usually the d. structure from D_params.m but could be used 
% seperately if needed. Randomization is done with functions 
...
%%% ADD NULL EVENTS
%%% Interleave null events (666 The Mark of the NULL!) according to nullratio and padd. 

function P = spm8w_realign(P,flags)
...
if length(P)==1,
    ...
    fprintf('Registering first image of each session to first image of session 1...\n'); %===DDW EDIT=== 
    ...
    for s=1:numel(P),
        fprintf('Registering remaining images for session %d...\n', s);                  %===DDW EDIT===   
...
fprintf('Registering Images:   ');                              %===DDW EDIT===
%spm_progress_bar('Init',length(P)-1,'Registering Images');     %===DDW EDIT===
...
for i=2:length(P),
    ...
    %spm_progress_bar('Set',i-1);                               %===DDW EDIT===
end;
fprintf('\n');                                                  %===DDW EDIT===
%spm_progress_bar('Clear');                                     %===DDW EDIT===
...
% Loop over images
%-----------------------------------------------------------------------
fprintf('Registering Images to Mean:   ');                        %===DDW EDIT===
%spm_progress_bar('Init',length(P),'Registering Images to Mean'); %===DDW EDIT===
for i=1:length(P),
    V  = smooth_vol(P(i),flags.interp,flags.wrap,flags.fwhm);
    d  = [size(V) 1 1 1];
    ss = Inf;
    ...
    fprintf([repmat('\b',1,3),'%3.0f'],i);                      %===DDW EDIT===
    %spm_progress_bar('Set',i);                                 %===DDW EDIT===
end;
fprintf('\n');                                                  %===DDW EDIT===
%spm_progress_bar('Clear');                                     %===DDW EDIT===
...
fg=spm_figure('GetWin','Graphics'); %'FindWin' %===DDW EDIT===FindWin won't spawn new fig
set(fg,'visible','off');                       %===DDW EDIT===Hide it
if ~isempty(fg),
    P = cat(1,P{:});
    ...
    x     = -0.08; %0.1                                 %===DDW EDIT===Pushed for more space
    y     =  0.9;
    for i = 1:min([numel(P) 12])
        text(x,y,[sprintf('%-4.0f',i) P(i).fname],'FontSize',9,'Interpreter','none','Parent',ax);
        y = y - 0.08;
    end
    if numel(P) > 12
        text(x,y,'................ etc','FontSize',9,'Parent',ax); 
    end

function spm8w_slice_timing(P, sliceorder, refslice, timing, prefix)
...
%SPMid = spm('FnBanner',mfilename,'$Rev: 4310 $');                       %===DDW EDIT===
%[Finter,Fgraph,CmdLine] = spm('FnUIsetup','Slice timing',0);            %===DDW EDIT===
fprintf('\n');                                                           %===DDW EDIT===
...
%spm('Pointer','Watch')                                          %===DDW EDIT===

for subj = 1:nsubjects
    task = ['Slice timing: working on session ' num2str(subj)];
    %spm('FigName',task,Finter,CmdLine);                         %===DDW EDIT===
    PP      = P{subj};
    Vin     = spm_vol(PP);
    nimgo   = numel(Vin);
    ...
    if nslices_t ~= nslices,
        ...
        %spm_progress_bar('Init',nslices,'Correcting acquisition delay','planes complete');         %===DDW EDIT===
        fprintf('Correcting acquisition delay for session %s... Planes complete:  ',num2str(subj)); %===DDW EDIT===
        ...
        for k = 1:nslices,
            ...
            %spm_progress_bar('Set',k);                      %===DDW EDIT===
            fprintf([repmat('\b',1,2),'%2.0f'],k);           %===DDW EDIT===
        end;
        %spm_progress_bar('Clear');                          %===DDW EDIT===
        fprintf('\n');                                       %===DDW EDIT===
    end
end
fprintf('Slice timing: done\n');                             %===DDW EDIT===
%spm('FigName','Slice timing: done',Finter,CmdLine);         %===DDW EDIT===
%spm('Pointer');                                             %===DDW EDIT===
